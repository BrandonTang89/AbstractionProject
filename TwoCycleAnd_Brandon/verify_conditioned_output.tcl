...
# Environmental Constraints via Conditioned Output and Restricted Indexing Relation
...
# Restrict the indexing relation to the input constraint
set index_rel_restricted [AND $index_rel $input_constraint]
# set index_rel_restricted $index_rel

# Check Coverage
# For using parametric encoding, we should cover all parameterised variables T'
# set coverage [satisfiesCoverage $index_rel_restricted $bdd_variables "" $input_constraint]
set coverage [satisfiesCoverage $index_rel_restricted $bdd_variables "" $input_constraint]
assert [expr {$coverage == 1}] "Indexing relation does not cover all cases"

# === Indexing Transformation ===
set transformed_ant_stimuli [strong_preimage_stim $antv $index_rel_restricted $bdd_variables]
# set transformed_ant_stimuli [strong_preimage_stim $antv $index_rel_restricted $bdd_variables]
...
set prop_low [weak_preimage $index_rel_restricted [FALSE] $bdd_variables]  
set prop_high [weak_preimage $index_rel_restricted $input_constraint $bdd_variables] 
