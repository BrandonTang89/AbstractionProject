...
namespace eval symsim {
    ...
    proc get_proof_id {property} {
        ...
        for {set i $max_id} {$i > 0} {incr i -1} {
            ...
            if {[string match *$name* $p] || [string match *$full_name* $p]} {
                set id $i
                break
            }
        }
    ...
    proc create_input_constraint {args} {
        ...
        array set options $args
        if {
            [info exists options(-property)] == 0 ||
    ...
    proc create_output_constraint {args} {
        ...
        # We use set rather than lappend for a dictionary
        dict set constraint $property $tick_list
    ...
    proc create_weakening_list {args} {
        ...
        array set options $args
        if {
            [info exists options(-instance)] == 0 ||
            [info exists options(-tick)] == 0
        } {
